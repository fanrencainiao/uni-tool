<template>
	<view class="">
		<tm-menubars title="我参与的" color="bg-gradient-primary-lighten" :showback="true"></tm-menubars>
		<tm-fullView>
				...
				<view class="grey text " :class="$tm.vx.state().tmVuetify.black ? 'black bk' : ''"
					...
					<view class="shadow-24 " >
						<tm-pullBottom  :height="height" :finish="finish" :disabled="disabled" :loading.sync="loading" @refresh="getdata">
							...
							<tm-switchList  v-for="(item,index) in datas" @click="todetail(item._id)" @actionsClick="actions($event,item._id)"  color="blue"  :rightLabel="item.createtime" rightIcon="icon-comment-dots" icon="icon-gem" :title="item.title || '已被发起者删除'" :label="item.des" :actions="item_1" :on='open'></tm-switchList>
						</tm-pullBottom> 
					</view>
		...
		methods: {
			...
			async list(){
				...
				await uniCloud.callFunction({
					name:"toupiao",
					data:{action:"listin",page:this.page,limit:this.limit,openid:uni.getStorageSync("openid")}
				}).then((res) =>{
